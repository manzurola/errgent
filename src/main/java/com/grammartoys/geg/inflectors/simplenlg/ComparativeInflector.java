...
public class ComparativeInflector extends SimpleNLGInflector {
    public ComparativeInflector(SimpleNLG simpleNLG) {
        super(simpleNLG);
    }
    ...
    protected boolean canHandle(Token token) {
        return token.matches(isAdverb().or(isAdjective()));
    }
}
