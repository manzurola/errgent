...
public class ErrgentTest {
    ...
    void singleInflection() {
        ...
        List<Inflection> inflections = generator.generateSingleError(target, REPLACEMENT_SUBJECT_VERB_AGREEMENT);
        Inflection inflection = inflections.get(0);

        List<Annotation> annotations = annotator.annotate(inflection.doc().tokens(), target.tokens())
                .stream()
        ...
        Assertions.assertEquals(expected, inflection.doc());
